...
class Auditor
{
    ...
    public static function logRequest( Request $request )
    {
        ...
        $_logInfo = array(
            ...
            '_path_translated'   => $request->server->get( 'http-path-translated' ),
            '_user_agent'        => $request->server->get( 'http-user-agent' ),
            '_request_timestamp' => $request->server->get( 'http-request-time-float' ),
        );
