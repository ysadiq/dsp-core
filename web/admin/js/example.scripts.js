...
//	users.list.js
//	The file contains a script to be run when the event 'users.list' is triggered

var ENABLE_ADD_PROPERTY = true;

_.each(event.data.record, function (record) {
    record.banned_for_life = (ENABLE_ADD_PROPERTY && (record.email == 'jerryablan@gmail.com'));
    print(record.email + ' is ' + ( record.banned_for_life ? '' : 'not') + ' banned for life.      ');
});

//apps.create.js  ## this is a post to /rest/app
