...
angular.module( 'ui.bootstrap.accordion' ).directive(
	'accordion', function() {
		return {
			restrict:    'EA',
...
angular.module( 'ui.bootstrap.accordion' ).directive(
	'accordionGroup', [
		'$parse', '$transition', '$timeout', function( $parse, $transition, $timeout ) {
			return {
...
angular.module( "ui.bootstrap.alert", [] ).directive(
	'alert', function() {
		return {
			restrict:    'EA',
...
dialogModule.controller(
	'MessageBoxController', [
		'$scope', 'dialog', 'model', function( $scope, dialog, model ) {
			...
			function( $http, $document, $compile, $rootScope, $controller, $templateCache, $q, $transition ) {
				...
				Dialog.prototype._loadResolves = function() {
					...
					if ( this.options.template ) {
						...
						templatePromise = $http.get( this.options.templateUrl, {cache: $templateCache} ).then(
							function( response ) {
					...
					angular.forEach(
						this.options.resolve || [], function( value, key ) {
							keys.push( key );
							values.push( value );
						}
					);
					...
					return $q.all( values ).then(
						function( values ) {
							var locals = {};
							angular.forEach(
								values, function( value, index ) {
				...
				return {
					...
					messageBox: function( title, message, buttons ) {
						return new Dialog(
							{templateUrl: 'template/dialog/message.html', controller: 'MessageBoxController', resolve: {model: {
								title:   title,
								message: message,
								buttons: buttons
							}}}
...
angular.module( 'ui.bootstrap.modal', [] ).directive(
	'modal', [
		'$parse', function( $parse ) {
			...
			return {
				...
				link:     function( scope, elm, attrs ) {
					...
					scope.$watch(
						shownExpr, function( isShown, oldShown ) {
							if ( isShown ) {
								open();
							}
							else {
...
angular.module( 'ui.bootstrap.pagination', [] )
	...
	'pagination', function() {
		return {
			...
			link:        function( scope ) {
				scope.$watch(
					'numPages + currentPage + maxSize', function() {
						scope.pages = [];
 ...
 */
angular.module( 'ui.bootstrap.popover', [] ).directive(
		...
		'$compile', '$timeout', '$parse', function( $compile, $timeout, $parse ) {
			...
			return {
				...
				link:  function( scope, element, attr ) {
					...
					attr.$observe(
						'popoverTitle', function( val ) {
							scope.tt_title = val;
						}
					);

					attr.$observe(
						'popoverPlacement', function( val ) {
					...
					function show() {
						...
						switch ( scope.tt_placement ) {
							case 'right':
								ttPosition = {
									top: (
										 position.top + position.height / 2 - ttHeight / 2
										 ) + 'px',
									left: (
 ...
 */
angular.module( 'ui.bootstrap.tooltip', [] ).directive(
 ...
 * @return {jQuery.promise}  A promise that is resolved when the transition finishes.
 */.factory(
	'$transition', [
