...
//        }
	...
	public function actionLogin( $redirected = false )
	{
		...
		$this->render( 'login',
			array(
				...
				'loginProviders' => ResourceStore::model( 'provider' )->findAll( 'is_login_provider = :is_login_provider AND is_active = :is_active',
	...
	public function actionSecurityQuestion()
	{
		...
		if ( isset( $_POST, $_POST['SecurityForm'] ) )
		{
			...
			if ( $_model->validate() )
			{
				try
				{
					$_result = Password::changePasswordBySecurityAnswer( $_model->email,
						$_model->answer,
						$_model->password,
						true,
						false );
		...
		if ( isset( $_POST, $_POST['ConfirmUserForm'] ) )
		{
			...
			if ( $_model->validate() )
			{
				try
				{
					...
					{
						case 'register':
							$_result = Register::userConfirm( $_model->email,
								$_model->code,
								$_model->password,
								true,
								false );
						...
						default:
							$_result = Password::changePasswordByCode( $_model->email,
								$_model->code,
								$_model->password,
								true,
								false );
	...
	/**
	 * Displays the script api tree
	 */
	public function actionApiReference()
	{
